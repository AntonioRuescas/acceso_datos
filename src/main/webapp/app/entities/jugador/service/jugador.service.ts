...
export class JugadorService {
  ...
  protected convertDateFromClient(jugador: IJugador): IJugador {
    return Object.assign({}, jugador, {
      fechaDeNacimiento: jugador.fechaDeNacimiento?.isValid() ? jugador.fechaDeNacimiento.format(DATE_FORMAT) : undefined,
    });
  }

  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.fechaDeNacimiento = res.body.fechaDeNacimiento ? dayjs(res.body.fechaDeNacimiento) : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((jugador: IJugador) => {
        jugador.fechaDeNacimiento = jugador.fechaDeNacimiento ? dayjs(jugador.fechaDeNacimiento) : undefined;
      });
    }
    return res;
  }
